...
<label class={cn('group/grid grid min-w-0 gap-1.5 text-[length:var(--app-text-sm)] text-[var(--nanke-ink)]', className)}>
  ...
  <textarea
    ...
    class={cn(
      'min-h-24 w-full min-w-0 resize-y rounded-[var(--nanke-radius-md)] border border-transparent bg-[var(--nanke-field)] px-3 py-2.5 text-[length:var(--app-text-sm)] leading-6 text-[var(--nanke-ink)] shadow-[var(--nanke-shadow-field)] outline-none transition-all duration-300 ease-out',
